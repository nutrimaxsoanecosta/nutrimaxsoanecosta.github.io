...
function QuizApp() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      if (!pacienteId) {
        setErrorMessage('O parâmetro pacienteId é obrigatório na URL (ex: ?pacienteId=UUID).');
        setLoading(false);
        return;
      }
      ...
      if (!baseUrl) {
        setErrorMessage('A variável de ambiente NEXT_PUBLIC_APPS_SCRIPT_URL não foi configurada.');
        setLoading(false);
        return;
      }
  ...
  const nomePacienteFormatado = paciente
...
export default function PacienteFormularioPage() {
  return (
